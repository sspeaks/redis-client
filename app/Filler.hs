...
fillCacheWithData :: (Client client) => Int -> RedisCommandClient (client 'Connected) ()
fillCacheWithData gb = do
  ...
  replicateM_ gb $ do
    _ <- liftIO (genRandomSet gen) >>= send client mempty
  ...
  evalStateT -- use state monad to keep track of the parse buffer and not drop input
    ( do
        replicateM_ numGbToRead $ do
          !res <- find isError <$> parseManyWith numKilosToPipeline (receive client mempty)
