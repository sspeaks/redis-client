...
data SendBlockingClient (a :: ConnectionStatus) where
  SendBlockingConnected :: !(MVar ()) -> !(MVar ()) -> !(MVar ByteString) -> SendBlockingClient 'Connected

instance Client SendBlockingClient where
  connect = error "SendBlockingClient: connect not supported"
  close _ = return ()
  send (SendBlockingConnected started release _) _ = liftIO $ do
    void $ tryPutMVar started ()
    takeMVar release
  receive (SendBlockingConnected _ _ replies) = liftIO $ takeMVar replies

createSendBlockingClient :: IO (SendBlockingClient 'Connected, IO (), IO ())
createSendBlockingClient = do
  started <- newEmptyMVar
  release <- newEmptyMVar
  replies <- newEmptyMVar
  let awaitStarted = do
        observed <- timeout 1000000 (takeMVar started)
        observed `shouldBe` Just ()
  return (SendBlockingConnected started release replies, awaitStarted, putMVar release ())

data FailingSendClient (a :: ConnectionStatus) where
  FailingSendConnected :: !(MVar ()) -> FailingSendClient 'Connected

instance Client FailingSendClient where
  connect = error "FailingSendClient: connect not supported"
  close _ = return ()
  send (FailingSendConnected started) _ = liftIO $ do
    void $ tryPutMVar started ()
    throwIO $ userError "send failed"
  receive _ = error "FailingSendClient: receive must not be called"

createFailingSendClient :: IO (FailingSendClient 'Connected, IO ())
createFailingSendClient = do
  started <- newEmptyMVar
  let awaitStarted = do
        observed <- timeout 1000000 (takeMVar started)
        observed `shouldBe` Just ()
  return (FailingSendConnected started, awaitStarted)
...
responseSlotSpec = describe "ResponseSlot" $ do
  ...
  it "completes a no-response command at send without consuming the next reply" $ do
    pool <- createSlotPool 16
    (client, awaitSend, reply) <- createBlockingClient
    mux <- createMultiplexer client (receive client)
    skipped <- submitCommandNoResponseAsync pool mux
      (encodeCmd ["CLIENT", "REPLY", "SKIP"])

    awaitSend
    timeout 1000000 (waitSlot pool skipped)
      `shouldReturn` Just (RespSimpleString "OK")

    next <- submitCommandAsync pool mux (encodeCmd ["PING"])
    awaitSend
    reply (encodeResp (RespSimpleString "PONG"))
    timeout 1000000 (waitSlot pool next)
      `shouldReturn` Just (RespSimpleString "PONG")
    destroyMultiplexer mux

  it "fails a no-response slot during teardown and permits subsequent pool reuse" $ do
    pool <- createSlotPool 16
    (client, awaitSend, _) <- createSendBlockingClient
    mux <- createMultiplexer client (receive client)
    skipped <- submitCommandNoResponseAsync pool mux
      (encodeCmd ["CLIENT", "REPLY", "SKIP"])
    ...
    awaitSend
    destroyMultiplexer mux
    failed <- try (waitSlot pool skipped) :: IO (Either SomeException RespData)
    ...
    reused <- submitCommandAsync pool reuseMux (encodeCmd ["PING"])
    addRecv (encodeResp (RespSimpleString "PONG"))
    timeout 1000000 (waitSlot pool reused)
      `shouldReturn` Just (RespSimpleString "PONG")
    destroyMultiplexer reuseMux

  it "preserves response ordering when a normal command pipelines behind SKIP" $ do
    pool <- createSlotPool 16
    (client, awaitSend, releaseSend) <- createSendBlockingClient
    mux <- createMultiplexer client (receive client)
    skipped <- submitCommandNoResponseAsync pool mux
      (encodeCmd ["CLIENT", "REPLY", "SKIP"])
    ...
    awaitSend
    releaseSend
    -- SKIP has no reader slot, so PONG must be delivered to PING.
    case client of
      SendBlockingConnected _ _ replies -> putMVar replies (encodeResp (RespSimpleString "PONG"))
    timeout 1000000 (waitSlot pool next)
      `shouldReturn` Just (RespSimpleString "PONG")
    destroyMultiplexer mux

  it "propagates send failure for a no-response command without a reader waiter" $ do
    pool <- createSlotPool 16
    (client, awaitSend) <- createFailingSendClient
    mux <- createMultiplexer client (receive client)
    skipped <- submitCommandNoResponseAsync pool mux
      (encodeCmd ["CLIENT", "REPLY", "SKIP"])
    ...
    result <- timeout 1000000
      (try (waitSlot pool skipped) :: IO (Either SomeException RespData))
    result `shouldSatisfy` \case
      Just (Left _) -> True
      _             -> False
    destroyMultiplexer mux

  it "reclaims a cancelled no-response wait after teardown" $ do
    pool <- createSlotPool 16
    (client, awaitSend, _) <- createSendBlockingClient
    mux <- createMultiplexer client (receive client)
    skipped <- submitCommandNoResponseAsync pool mux
      (encodeCmd ["CLIENT", "REPLY", "SKIP"])
    waiter <- async (waitSlot pool skipped)
